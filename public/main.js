...
//does not allow set interval if its already running
...
let websocketReplacer = async () => {
   ...
   for (let msg of history) {
      if (msg.message == "***deleted***") {
         let msgToBeRemoved = document.getElementById(`${msg.id}`)
         if (msgToBeRemoved)
            msgToBeRemoved.parentNode.removeChild(msgToBeRemoved)
         continue
      }
...
//clearing and resetting everything
...
msgForm.onsubmit = async event => {
   event.preventDefault()
   //cant send anything if you offline =)
...
//helps to make image send label informative
...
changeAvatarBtn.onclick = async () => {
   ...
   if (uploadedFiles.length) {
      ...
      fileReader.onload = async event => {
         ...
         let ownMessages = [
            ...document.getElementsByClassName(`${currentUser.name}`)
         ]
         ownMessages.forEach(elem => {
...
userSearch.oninput = () => {
   ...
   for (let user of users)
      if (
         ...
         user.name.startsWith(userSearch.value)
      )
